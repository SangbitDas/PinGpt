...
class UIService {
    ...
    async insertPinUnpinButton(popperElement) {
        ...
        console.log("🔍 PinGPTChat: Extracted chat info:");
        console.log("  Chat ID:", chatId);
        console.log("  Chat Name:", chatName);
        console.log("  Chat Link:", chatLinkElement.href);

        if (!chatId || !chatName) {
            console.log("❌ PinGPTChat: Missing chat ID or name");
            return;
        }

        const templateItem = menu.querySelector('[role="menuitem"]');
        if (!templateItem) {
            console.log("❌ PinGPTChat: No menu item template found");
            return;
        }

        console.log("✅ PinGPTChat: Creating pin button...");
        ...
        pinButton.onclick = async (event) => {
            ...
            console.log("🔥 PinGPTChat: Pin button clicked!");
            console.log("Chat ID:", chatId);
            console.log("Chat Name:", chatName);

            try {
                console.log("🔄 PinGPTChat: Toggling chat pin...");
                await this.dbService.toggleChatPin(chatId, chatName);
                console.log("✅ PinGPTChat: Chat pin toggled successfully");

                // Update button text and icon to reflect new state
                const isPinned = await this.dbService.isPinned(chatId);
                textSpan.textContent = isPinned ? i18n.unpin : i18n.pin;
                iconDiv.innerHTML = isPinned ? SVG_unpin : SVG_pin;
                console.log("🔄 PinGPTChat: Button updated to show", isPinned ? "unpin" : "pin");

                // Close the menu
                triggerButton.click();
                console.log("🔒 PinGPTChat: Menu closed");
            } catch (error) {
                console.error("❌ PinGPTChat: Error toggling chat pin:", error);
            }
        };
    }
     ...
     */
    updatePinnedChats() {
        console.log("🔄 PinGPTChat: Updating pinned chats display...");
        this.dbService.getPinnedChats().then((pinnedChats) => {
            console.log("📋 PinGPTChat: Current pinned chats:", pinnedChats);
...
class DBService {
    ...
    async getPinnedChats() {
        console.log("📚 PinGPTChat: Getting pinned chats from storage...");
        const result = await chrome.storage.sync.get(DBService.PINNED_CHATS_KEY);
        const chats = result[DBService.PINNED_CHATS_KEY] || [];
        console.log("📚 PinGPTChat: Retrieved chats:", chats);
        return chats;
    }
    setPinnedChats(chats) {
        console.log("💾 PinGPTChat: Setting pinned chats:", chats);
        chrome.storage.sync.set({ [DBService.PINNED_CHATS_KEY]: chats });
    }

    // Category management functions
    async getCategories() {
        console.log("📂 PinGPTChat: Getting categories from storage...");
        ...
        console.log("📂 PinGPTChat: Retrieved categories:", categories);
        return categories;
    }

    async setCategories(categories) {
        console.log("💾 PinGPTChat: Setting categories:", categories);
    ...
    async getChatCategories() {
        console.log("📋 PinGPTChat: Getting chat categories from storage...");
        ...
        console.log("📋 PinGPTChat: Retrieved chat categories:", chatCategories);
        return chatCategories;
    }

    async setChatCategories(chatCategories) {
        console.log("💾 PinGPTChat: Setting chat categories:", chatCategories);
    ...
    async toggleChatPin(id, name) {
        console.log("🔄 PinGPTChat: Checking if chat is pinned...");
        const isPinned = await this.isPinned(id);
        console.log("📌 PinGPTChat: Chat is pinned:", isPinned);

        if (isPinned) {
            console.log("🔄 PinGPTChat: Unpinning chat...");
            await this.unpinChat(id);
            console.log("✅ PinGPTChat: Chat unpinned");
        } else {
            console.log("🔄 PinGPTChat: Pinning chat...");
            await this.pinChat(id, name);
            console.log("✅ PinGPTChat: Chat pinned");
        }
    }
...
dbService.getCategories().then(categories => {
    console.log("📂 PinGPTChat: Categories initialized:", categories);
});
